    ...
    Directory(path.join(_targetPath, 'my_app'))
      ...
      try {
        ...
        file = await file.writeAsString(
          contents
              .replaceAll('starter_app', '{{project_name.snakeCase()}}')
              .replaceAll('starter-app', '{{project_name.paramCase()}}')
              .replaceAll('A new Flutter project.', '{{{description}}}')
              .replaceAll('Starter App', '{{project_name.titleCase()}}')
              .replaceApplicationId(file.path),
        );

        final fileSegments = file.path.split('/').sublist(2);
        if (fileSegments.contains('starter_app')) {
